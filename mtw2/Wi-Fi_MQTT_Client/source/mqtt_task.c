...
void mqtt_client_task(void *pvParameters)
{
        ...
        if (pdTRUE == xQueueReceive(mqtt_task_q, &mqtt_status, portMAX_DELAY))
        {
            ...
            switch(mqtt_status)
            {
                ...
                case HANDLE_DISCONNECTION:
                {
                    ...
                    printf("\nInitiating Reconnection to MQTT...\n");
     ...
     */
    exit_cleanup:
    printf("\nTerminating Publisher and Subscriber...\n");
...
static cy_rslt_t wifi_connect(void)
{
    ...
    if (cy_wcm_is_connected_to_ap() == 0)
    {
        // List of SSIDs and passwords to try
        const char* ssids[] = {WIFI_SSID, WIFI_SSID_2, WIFI_SSID_3};
        const char* passwords[] = {WIFI_PASSWORD, WIFI_PASSWORD_2, WIFI_PASSWORD_3};
        ...
        {
            memcpy(connect_param.ap_credentials.SSID, ssids[i], sizeof(connect_param.ap_credentials.SSID));
            memcpy(connect_param.ap_credentials.password, passwords[i], sizeof(connect_param.ap_credentials.password));
            ...
            for (uint32_t retry_count = 0; retry_count < MAX_WIFI_CONN_RETRIES; retry_count++)
            {
                ...
                printf("Failed to connect. Error code:0x%0X. Retrying in %d ms. Retries left: %d\n",
        ...
        printf("\nMaximum Wi-Fi connection attempts has been reached!\n");
